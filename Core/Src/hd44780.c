...
static void LCD_Init(int rows, int fourbit, uint16_t data_pin[], GPIO_TypeDef* ports[], struct LCD *self)
{
    for (int i = 0; i < 8; i++)
    {
...
static void LCD_Create_Char(uint8_t location, uint8_t charmap[], struct LCD *self)
{
    location &= 0x7;
...
static void LCD_Command(uint8_t cmd, struct LCD *self)
{
    send(cmd, 0, self);
}

static void LCD_Write(uint8_t value, struct LCD *self)
{
    send(value, 1, self);
}
